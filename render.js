...
function appendData(roomName, roomCode){
    ...
    append({name: "Watch", content: `Welcome to ${roomName}`, pfp: "#f3dfbf"})
    append({name: "Watch", content: `Share the room code (${roomCode}) with others to invite them to the party.`, pfp: "#f3dfbf"})
    append({name: "Watch", content: "They would need to have the same video file with them to join this watch party.", pfp: "#f3dfbf"})
            ...
            if(localStorage.getItem("videoPath") == null || localStorage.getItem("videoSize") == null || document.getElementById("join-username").value.length == 0) {
                ...
                .then( async (result) => {
                    ...
                    if(resp.message != "success") {
                        ...
                        document.title = `Watch | ${resp.roomName}`
                        roomPage.style.display = "block"
                    }   
                })
                .catch(error => console.log('error', error));
